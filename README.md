...
**Force 2 — The entropy bonus** wants $\sigma$ **large**. Entropy measures how "spread out" the bell curves are — wide curves = high entropy = lots of randomness, narrow curves = low entropy = predictable. The entropy bonus is an extra term added to the loss function that **rewards** the agent for keeping entropy high. In the code ([line 1124](rl_weight_agent.py#L1124)):
```python
