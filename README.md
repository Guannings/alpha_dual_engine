...
**Does $\sigma$ ever reach zero?** No — but there's no hard floor preventing it either. What stops $\sigma$ from collapsing is a **tug-of-war** inside the loss function ([`rl_weight_agent.py:1127`](rl_weight_agent.py#L1127)). One force (the policy gradient) wants $\sigma$ small — once you've found good allocations, randomness just adds noise. The other force (the **entropy bonus**, controlled by `ent_coef = 0.10` at [line 1080](rl_weight_agent.py#L1080)) wants $\sigma$ large — it penalises the loss whenever entropy drops, which happens when $\sigma$ shrinks. $\sigma$ settles at whatever equilibrium these two forces reach: small enough to mostly exploit what works, large enough to keep some exploration alive. In practice, $\sigma$ starts at 1.0 ([line 451](rl_weight_agent.py#L451): `log_std` initialized to 0, so $e^0 = 1.0$) and shrinks during training but never collapses to zero.
