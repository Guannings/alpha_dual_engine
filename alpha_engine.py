...
# Pre-trained classifier artifact, regenerated by tools/refresh_artifacts.py
# (and weekly by CI). Cold starts load it in seconds instead of retraining;
# only the refresh script ever WRITES it, so normal runs keep git clean.
ARTIFACT_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "artifacts", "classifier_cache.joblib")
ARTIFACT_MAX_AGE_DAYS = 10


def _load_classifier_artifact(features: pd.DataFrame):
    """Return (classifier, ml_probs) from the committed artifact, or None if
    it is missing, stale, schema-mismatched, or unreadable (e.g. pickled
    under an incompatible library version) — every failure falls back to a
    normal from-scratch training."""
    try:
        if not os.path.exists(ARTIFACT_PATH):
            return None
        payload = joblib.load(ARTIFACT_PATH)
        age_days = (pd.Timestamp.utcnow() - pd.Timestamp(payload["generated"])).days
        if age_days > ARTIFACT_MAX_AGE_DAYS:
            logger.info(f"Classifier artifact is {age_days}d old — retraining.")
            return None
        classifier = payload["classifier"]
        if classifier.feature_names != features.columns.tolist():
            logger.info("Classifier artifact feature schema changed — retraining.")
            return None
        # The artifact ends at its generation date; carry the last known
        # probability forward over the few trading days since (probs move slowly)
        ml_probs = payload["ml_probs"].reindex(features.index).ffill()
        logger.info(f"Loaded pre-trained classifier artifact ({age_days}d old).")
        return classifier, ml_probs
    except Exception as e:
        logger.warning(f"Classifier artifact unusable ({e}) — retraining.")
        return None


def save_classifier_artifact(classifier, ml_probs: pd.Series) -> str:
    """Write the artifact. Called by tools/refresh_artifacts.py only."""
...
def train_classifier_cached(_dm: DataManager, _returns: pd.DataFrame, _features: pd.DataFrame):
    ...
    cached = _load_classifier_artifact(_features)
...
def main():
    ...
    # Shallow-copy before assigning config: cache_resource returns the SAME
    # object to every session, so mutating it directly would let concurrent
    # sessions with different slider values overwrite each other's config.
        ...
        with tab4:
            ...
            if run_ablation:
                with st.spinner("Running ablation backtests (Random + Equal weight baselines)..."), _COMPUTE_LOCK:
                    ...
                    class _RandomModel:
                        # Self-contained generator: reseeding the GLOBAL
                        # np.random here would silently change every other
                        # random consumer that runs later in the script.
                        def __init__(self):
                            self._rng = np.random.default_rng(42)

                        def predict_deterministic(self, obs):
                            return self._rng.dirichlet(np.ones(12))
